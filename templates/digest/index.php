...
<div class="screen-head">
  <div>
    <h1><?= View::t('digest.title') ?></h1>
    <p class="screen-sub"><?= View::t('digest.subtitle_1') ?>
      <span class="mono"><?= View::e($date) ?></span> <?= View::t('digest.subtitle_2') ?></p>
  </div>
...
<div class="callout callout--warn callout--banner" role="alert">
  ...
  <div><strong><?= View::t('digest.fallback_title') ?></strong> <?= View::t('digest.fallback_body') ?> <a href="/settings"><?= View::t('nav.settings') ?></a>.</div>
</div>
<?php endif; ?>
  ...
  <div class="card__body">
    ...
    <ul class="job-list">
      ...
      <li class="job-row">
        ...
        <a class="btn btn--ghost btn--sm" href="/runs/<?= (int) $item['run_id'] ?>"><?= View::t('common.view_run') ?></a>
      </li>
      <?php endforeach; ?>
    </ul>
    <?php endif; ?>
...
<div class="card">
  <div class="card__head"><h2><?= View::t('digest.auto_published') ?></h2>
    <span class="card__action"><span class="chip chip--faint num"><?= count($digest['auto_published']) ?></span></span>
  </div>
  <div class="card__body">
    <?php if ($digest['auto_published'] === []): ?>
    <p class="muted"><?= View::t('digest.none_auto_published') ?></p>
    <?php else: ?>
    <ul class="job-list">
      <?php foreach ($digest['auto_published'] as $item): ?>
      <li class="job-row">
        <div class="job-row__main">
          <span class="job-row__type mono"><?= View::t('digest.publish_n', ['n' => (int) $item['id']]) ?> · <?= View::t('common.run_n', ['n' => (int) $item['run_id']]) ?></span>
          <span class="job-row__entity"><?= View::t('digest.published_to', ['n' => (int) ($item['result']['published'] ?? 0), 'total' => (int) ($item['result']['posts'] ?? 0)]) ?>
            <?php if ((int) ($item['result']['failed'] ?? 0) > 0): ?> · <?= View::t('digest.n_failed', ['n' => (int) $item['result']['failed']]) ?><?php endif; ?>
...
<div class="card">
  <div class="card__head"><h2><?= View::t('digest.guardrail_events') ?></h2>
  ...
  <div class="card__body">
    ...
    <p class="muted"><?= View::t('digest.none_guardrail') ?></p>
    <?php else: ?>
    <div class="tl">
