...
- Tarih: 2026-06-13
- Güncelleyen: Claude (**`/go` SÜREKLİ LOOP — FAZ 16+17+18+19 KABUL + commit'li (her biri kendi branch'inde, stack).
  SIRADA: FAZ 20 (cila/perf/a11y — SON faz).** Faz 19 = Canlı Katman / SSE — **tek gerçek backend yüzeyi**,
  güvenli. Tasarım: **IMMEDIATE-CLOSE SSE** (`/live` tek snapshot + `retry:` → kapat; EventSource ~5s'de yeniden
  bağlanır = canlı tick). Held-connection YOK → php -S/harness'ı bloke etmez, session lock tutmaz, uzun
  transaction yok, kaynak tükenmesi yok (= güvenlik artısı). Yeni: `LiveController::stream` (auth-guard + tenant
  `workspace->id()` + `Cockpit::liveSnapshot` [tek workspace-scope SELECT, read-only] + `session_write_close` +
  text/event-stream Response, request param YOK), `GET /live` ($protected), `live-client.js` (EventSource→snapshot
  → topbar `.is-live` + awaiting KPI textContent; EventSource yoksa sessiz fallback). `app.php` topbar live
  göstergesi (heartbeat dot + "Canlı · az önce" / html.js-gated → no-JS GİZLİ, dürüst), `dashboard.php`
  `data-live-awaiting`, `app.css` F19 bloğu (heartbeat opacity pl-hb reuse + reduced-motion + ≤640 text gizle/≤460
  tüm gizle), `lang/en+tr` +2 `live.*` parite, `tests/run.php` +8. **761 PASS/0 FAIL**; visual **69 PNG / 0 err /
  0 overflow / exit 0** (immediate-close SSE harness'ı bloke ETMEDİ → H2 risk çözüldü). **3 gate PASS/0 blocker**
  (security AĞIR: tenant-izol + no-DoS/no-held-conn + read-only + session_write_close, 2 LOW: router-level unauth
  test + enable-time Caddy/CF no-cache → takip; ux: §1.2 temiz heartbeat + no-JS dürüst gizle; qa: scope/parite/
  build-free/JS-off). Branch `feat/phase-19-live-sse` (F18'den stack). [F18 `1a4486d`, F17 `231b709`, F16 `8a6f561`].)

## Mevcut durum (kaldığımız yer)

- Aşama: **`/go` SÜREKLİ LOOP — FAZ 16+17+18+19 KABUL + commit'li** (her faz kendi branch'inde, STACK; push YOK).
  Loop branch'leri STACK eder: F20 (SON) → `feat/phase-19-live-sse`'ten branch'lenir (önceki fazlar temel).
  Run sonunda kullanıcı tüm stack'i inceler, beğenmediği fazı `git revert <faz-commit>` ile geri alır.
  Branch'ler: F19 `feat/phase-19-live-sse` (F18'den), F18 `feat/phase-18-pipeline-viz` (F17'den), F17
  `feat/phase-17-signature-dashboard` (F16'dan), F16 `feat/phase-16-motion-core` (main'den). Commit'ler: F18
  `1a4486d`, F17 `231b709`, F16 `8a6f561`. origin/main `ec795ba` (loop push yapmaz).
...
- **Açık takip (genel) → F20'de topla:** pre-existing jargon (`nav.foot_title` "Phase 12 · …", worker banner,
  node-id mono etiketler queue/runs'ta) → honest-copy; JobRepo `SELECT *` read-model (LOW); 768px pipeline chip-adı
  truncation (cosmetic); SSE router-level unauth test (LOW, defense-in-depth zaten 2 katman); inline `<video>`
  no-native-controls JS-off (view-run linki var, kabul). **ENABLE-TIME ops:** Caddy/CF `text/event-stream` no-cache.
...
0. **`/go` LOOP DEVAM → SIRADA FAZ 20 (SON faz)** (Cila, Performans & Erişilebilirlik kapanışı): **perf** (boşta
   GPU ~0, 60fps; §1.2 ihlali taraması: kalıcı backdrop/animasyonlu blur/dönen öğe = sıfır — Experience ekranları),
   **a11y** (WCAG AA kontrast incl. faint-tier `phase-15-followups.md` A11Y borçları; klavye nav: ⌘K/drawer/focus
   trap; `aria-current`/SR; focus-visible), **dürüst rozet SON denetimi** (UI + her yer), **güvenlik SON geçişi**
   (Experience yüzeyleri), prefers-reduced-motion TAM kapsama. **+ genel açık takipleri TEMİZLE** (yukarıdaki
   "Açık takip" listesi: `nav.foot_title` "Phase 12" jargon, worker banner, node-id mono etiketler, JobRepo
   SELECT*, 768px chip truncation, SSE router-test). Spec: `experience-layer-plan.md` §8. **Branch:
   `feat/phase-19-live-sse`'ten `feat/phase-20-<slug>` aç** (stack). **Koşullu compliance gate** (dürüst rozet).
   Reviewer: ux + security (+ compliance). Faz 20 bitince **(H3) loop DURUR** → tüm faz verdict+branch listesi sun.
