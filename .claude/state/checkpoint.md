...
- Tarih: 2026-06-15
- Güncelleyen: Claude (**ZERNIO accountId FIX (`2a2de5c`, push'lu) — gerçek publish hazır.** Bug: publish `400 Invalid
  accountId format`; POST /posts `platforms[].accountId = accounts.external_ref` ama connect bunu UYDURUYORDU
  (`zacct_`+random) — gerçek Zernio SocialAccount `_id` (24-hex, GET /accounts) yerine. FIX (adapter/controller/data;
  engine/şema/node-graph DEĞİŞMEDİ): `accounts()` PublishProvider arayüzüne taşındı (Mock+Spy impl);
  `AccountRepository::setExternalRef`; `connectCallback` gerçek _id'yi `accounts()` ile çözüyor (platform+@/case-insens.
  username; eşleşmezse fallback); yeni `sync()` + `POST /accounts/sync` + "Hesapları eşitle" butonu reconcile. UI
  vendor-neutral (jargon-gate: "Zernio" UI'dan çıkarıldı). **839 PASS/0 FAIL (+7)**; secret yok; 10 dosya. **CANLI
  UÇTAN UCA KANIT (yayın YOK):** GET /accounts `_id=6a2f250a5f7d1751abb4803a` → ws#2 #3 external_ref reconcile (WAL-safe
  yedek) → gerçek `postPayload` `accountId`=o _id (MATCH, 24-hex). ws#2 #3 data-fix UYGULANDI → panelden gerçek publish
  retry hazır. **`.env`:** OPENAI/TTS/STOCK_MOCK=false, STORAGE_DRIVER=r2, **ZERNIO_MOCK=false (gerçek yayın ON)**,
  VIDEO_MOCK=true. **AÇIK:** worker PID 34294 fix'lerden önce başlamış olabilir → `php bin/worker.php` RESTART önerilir.
...
- Aşama: **ZERNIO accountId FIX push'lu (`2a2de5c`) — gerçek publish hazır.** main lineer, **839 PASS / 0 FAIL**.
