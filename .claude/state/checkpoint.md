...
- Aşama: **GENERATION STACK GERÇEK + TTS WAV-fix push'lu (`6b0c56f`); FAZ 10 (Zernio) KABUL/COMMIT/PUSH.**
  main lineer, **825 PASS / 0 FAIL**. **`.env` (gitignored, GERÇEK PARA):** OPENAI_MOCK=false, TTS_MOCK=false,
  STOCK_MOCK=false, STORAGE_DRIVER=r2; **ZERNIO_MOCK=true + VIDEO_MOCK=true** (yayın + AI-video MOCK; gerçek yayın
  kullanıcının ayrı kararı). Bütçe cap'leri PreflightGate ile etkin. `.env` yedeği `.env.bak.pre-gen-20260615T012753Z`.
  Faz 10: Zernio gerçek adapter commit `6891f8b` + 0013 dev DB'ye uygulandı (migration=13, /settings 500 çözüldü).
  Önceki: EXPERIENCE LAYER (FAZ 16–21) main'e merge+push (`806fdf8`), visual baseline `storage/visual/phase-21-rev/`.
