...
- Tarih: 2026-08-27
- Güncelleyen: Claude (**İŞARETLEME TURU — `9cd3a67` push'lu, 1122 PASS/0 FAIL.
  /trends skorları + tazelik rozeti işaretlendi (batch provenance, fail-closed),
  COST PER CONTENT demo harcamadan arındırıldı ($0.01→$0.00 = 7 gerçek kuruş /
  17 gerçek render). **compliance gate NO-GO verdi ve haklıydı:** aynı uydurma
  sayılar İKİ yüzeyde daha işaretsizdi — hesap kartındaki `+N today` ve panel
  drawer'ındaki trend skoru; ikisi de kapatıldı. Vitrin ws2'de KURULU,
  ön koşullar GEÇİCİ.**)
- Önceki: Claude (**BİTİŞ TURU — `142c552`: fixture seed, K1, approve-guard,
  run sayfasında video.**)
...
- 2026-08-27 — **İŞARETLEME TURU (2 iş → 4 düzeltme) — `9cd3a67` push'lu, 1122 PASS/0 FAIL, gate 99 PNG temiz.** (1) **/trends işaretlendi:** her skorun yanında `sample`/`örnek` çipi, tazelik rozetinden ÖNCE ikinci bir çip (böylece "fresh · 11 dk önce" ancak "bu örnek satırlar 11 dk önce üretildi" diye okunur), ızgara altında not. Kaynak **batch'in kendi `source`'u**, `TREND_MOCK` env bayrağı DEĞİL — önbellekteki satırlar kendilerini getiren sağlayıcıyı taşır, yani gerçek batch bayrak açılınca da işaretsiz kalır, mock batch kapanınca da işaretli. **Fail-closed:** test "gerçek dünyayı okuyan sağlayıcılardan biri mi?" diye soruyor, yoksa `mock` dışında adlandırılmış her yeni stub varsayılan olarak ölçüm gibi görünürdü. (2) **COST PER CONTENT:** manifest'li satırlar **iki taraftan da** çıkarıldı (gerçek para şişmiş render sayısına bölünürse maliyet olduğundan DÜŞÜK görünür). `sqlite_master` kontrolüyle korumalı — eksik tablo PREPARE'de patlar ve kozmetik bir ortalama için bütün paneli düşürürdü (ADR-024'ün yazıldığı `slot_occurrences` şekli). Canlı: $0.01 → $0.00 = 7 gerçek kuruş / 17 gerçek render. `$kpis['renders']` diğer çağıranlar için değişmedi. **compliance gate NO-GO — haklıydı:** (a) hesap kartındaki `+N today` üç tur boyunca uydurma ve İŞARETSİZDİ; komşunun çipini ödünç alamıyor çünkü `.acc-card__foot` `space-between` (çip ortada ada olur, yeşil sayı uzak kenarda tek başına) ve aynı ekranın dipnotu "işareti OLMAYAN sayı bağlı hesaptan gelir" diyor → kendi çipini aldı; (b) mock trend skoru panelin üretim hattı **drawer**'ında `Score 88 · tech · EN` diye işaretsiz tekrar çıkıyordu, hem de duvarın "gerçek hiçbir şeyi sıralamıyor" dediği sayı, bir tık ötede → `TrendExecutor` provenance'ı zaten aynı diziye yazıyordu, işaretlendi. **Yeni harness körlüğü kaydedildi: görsel gate hiçbir drawer'ı AÇMIYOR**, o yüzden içindeki işaretsiz skor 99 PNG'de hiç fotoğraflanmadı ve tam bu konuya ayrılmış turu atlattı. **4 sabotaj kanıtı.** Ayrıca manifest tablo adı artık bağlı parametre (security.md). Ertelenenler: `visual-seed` ölçmediği süre/boyutu yazıyor, `spent_mtd`/bütçe kapısı filtresiz (bugün yalnız seed'in tarihlemesi yüzünden dürüst), `$0.00` "bedava" gibi okunuyor.
