...
- Tarih: 2026-06-14
- Güncelleyen: Claude (**FAZ 21 — İLK SUNUM REDDEDİLDİ (commit `e0f2541`) → 6 MADDELİK DÜZELTME BİTTİ (commit
...
- Aşama: **FAZ 16→21 HEPSİ BİTTİ + commit'li; İNSAN KAPISI bekliyor** (6 stacked branch; push/merge YOK).
  **STACK (en üst = F21, hepsini içerir):** `feat/phase-21-full-conversion` → `feat/phase-20-polish-a11y` →
  ...
  `feat/phase-16-motion-core` → main. **Commit'ler:** F21 tip `cc8df98` (review fixes) → `e0f2541` (ilk F21),
  F20 `fd80455`, F19 `460e487`, F18 `1a4486d`, F17 `231b709`, F16 `8a6f561`. origin/main `ec795ba` (PUSH YAPILMADI).
  **İNCELEME:** her faz kendi commit'inde; beğenmediğin fazı `git revert <commit>` ile geri al; main'e merge insan
  onayıyla. Test: **776 PASS**. Visual baseline: `storage/visual/phase-21/` (69 PNG, tüm 12 ekran v3).
...
0. **FAZ 21 BİTTİ (1. sunum reddedildi → 6 düzeltme yapıldı) — branch-commit'li, İNSAN KAPISI BEKLİYOR.** Branch
   `feat/phase-21-full-conversion` tip `cc8df98` (F20'den stack; PUSH/MERGE YOK). Kullanıcı 12 ekranı baştan sona
   gezip onaylar (screenshot `storage/visual/phase-21/`). Onaylanınca → 16→21 stack'i main'e merge **insan onayıyla**;
   beğenilmeyen faz `git revert <commit>`. **776 PASS, 4 gate GO/0 blocker.** Spec `phase-21-full-conversion.md`.
   Açık takip (ertelendi, kozmetik): trends freshness chip ham ISO timestamp; workflows/show+runs/show NODE-TRACK
   canonical adları (graph = canonical-vocabulary evi, kasıtlı). Not: /logs gerçek-DB event'leri artık humanize.
