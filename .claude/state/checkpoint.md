...
- Tarih: 2026-06-15
- Güncelleyen: Claude (**ASSEMBLY R2-STAGING FIX (`62c76fe`, push'lu) + ws#2 ölü-asset temizliği + GERÇEK YAYIN AÇIK.**
  Bug: STORAGE_DRIVER=r2'de ffmpeg girdisi (R2'ye taşınmış/evicted) yerelde yok → "No such file (exit 254)" tüm
  run'ları blokluyordu. FIX (asset-resolution katmanı; engine/şema/node-graph DEĞİŞMEDİ): `AssemblyEngine::localInput()`
  + `AssetCache::remember()` HIT self-heal → her ffmpeg girdisi her storage modunda yerel garantili (R2'den
  stage / eksikse restore/re-produce); kurtarılamazsa kriptik çökme yerine dürüst FfmpegException. **832 PASS/0 FAIL
  (+7)**; secret yok. **#13/#18 retry** ASIL nedeni ortaya çıkardı: referans asset #3 "Smoke clip" baytları
  GERÇEKTEN KAYIP (storage_disk=local + yerel YOK + R2'de YOK) → kodla kurtarılamaz. **Temizlik (salt veri, WAL-safe
  yedek):** #13/#18 dead-lettered; ws#2 avatar(=3)→NULL; asset #3 hard-delete → ws#2 0 ready asset. **#4 CANLI KANIT:**
  `AssetFetchExecutor` ws#2 faceless+face → `source=stock/provider=pexels`, 5.3MB klip indi (ölü-avatar yolu elendi).
  **`.env`:** OPENAI/TTS/STOCK_MOCK=false, STORAGE_DRIVER=r2, **ZERNIO_MOCK=false (gerçek yayın ON)**, VIDEO_MOCK=true.
  **AÇIK:** worker PID 34294 fix'ten önce başlamış olabilir → kullanıcı `php bin/worker.php` RESTART etmeli. Detaylar
  ↓ oturum-log. Önceki: FAZ 10 Zernio (`6891f8b`) + generation/TTS-fix (`6b0c56f`).)

## Mevcut durum (kaldığımız yer)

- Aşama: **ASSEMBLY R2-STAGING FIX push'lu (`62c76fe`) + ws#2 ölü-asset temizliği.** main lineer, **832 PASS / 0 FAIL**.
  `AssemblyEngine::localInput()` + `AssetCache` HIT self-heal → ffmpeg girdileri her storage modunda yerel garantili
  (R2'den stage / eksikse restore/re-produce). **`.env` (gitignored, GERÇEK PARA + GERÇEK YAYIN AÇIK):** OPENAI_MOCK=false,
  TTS_MOCK=false, STOCK_MOCK=false, STORAGE_DRIVER=r2, **ZERNIO_MOCK=false** (gerçek yayın ON — IG @ai.neeidy doğrulandı,
  manual onay, 0 bekleyen), VIDEO_MOCK=true. Bütçe cap PreflightGate. **ws#2 temizlik (salt veri):** runs #13/#18
  dead-lettered (kaynak asset baytları kayıp); ölü library asset #3 "Smoke clip" silindi + ws#2 avatar=NULL → ws#2
  0 ready asset → yeni run'lar STOCK'a düşer (canlı Pexels proof: faceless+face → source=stock/pexels, 5.3MB klip indi).
  **AÇIK:** çalışan worker PID 34294 fix'ten önce başlamış olabilir → kullanıcı `php bin/worker.php`'i RESTART etmeli.
  Faz 10 Zernio commit `6891f8b`; EXPERIENCE LAYER (16–21) `806fdf8`.
