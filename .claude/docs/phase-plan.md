...
Goals: local run+seed script; headless screenshot tool (375/768/1280 × EN/TR, console-error capture; dev-only Node tooling — app stays build-free); `/goal` autonomous loop command (plan→build→3-gate test→verdict→branch-commit→/clear; fail-cap 2 → stop-and-report; human-gate table); orchestrator + ux-reviewer/qa-reviewer/security-auditor gate task templates. NO product UI/DB/route change. Full spec: `.claude/docs/experience-layer-plan.md` §2–3. Human gate: YES.
...
Non-goals: SSE/live, new backend, new screen, pipeline node-graph (P18), inline player (P17). Acceptance: motion premium + state-mapped; reduced-motion respected; 60fps, idle GPU ~0; Cmd+K + drawer work; §1.2 clean; full suite green; TR/EN intact. Human gate: YES. ux-reviewer.
...
Non-goals: pipeline node-graph (P18), SSE liveness (P19), new metrics backend. Acceptance: dashboard renders from real data; missing data → "no data"; inline player plays in-card; badges truthful; empty/loading/error present; 60fps; 375/768/1280 OK; no regression. Human gate: YES. ux-reviewer + compliance-reviewer.
...
Non-goals: workflow engine change, SSE live progress (P19), new node type. Acceptance: real pipeline state reflected (done/active/wait); fill-flow only on active segment; box click opens correct plain panel; no jargon; mobile stacked works; §1.2 clean; engine untouched; no regression. Human gate: YES. ux-reviewer.
...
Non-goals: new features. Acceptance: perf targets met + measured; a11y AA; all badges truthful; security clean; reduced-motion 100%; no regression. Human gate: optional. ux-reviewer + security-auditor (+ compliance-reviewer).
