...
## Phase 24 — Weekly Plan as a Calendar, two modes · token: START PHASE 24
Why: Phase 23's weekly times are a TEMPLATE that cannot hold anything — content is bound to a time only at the moment of approval, one item at a time. "Put this video on Tuesday the 24th" has no home, and there is no notion of who fills a time.
Goals: dated calendar cells (migration 0017 `slot_occurrences`, identity = slot × LOCAL day so DST moves the instant without splitting a day) + a mode per publishing time (`publish_slots.mode`: you add the video / Kuyash makes one); assignment starting a real run whose planned instant is written at BIRTH (so it survives an approval that names nothing AND the compliance agent's auto-approval path); `PlanRunner` worker chore — materialize, sweep past times beyond a 60-minute grace, produce automatic content inside its lead window behind every existing guardrail (pause, kill switch, per-account daily cap, owner, workflow, budget preflight); calendar-first `/plan` (day list at 375px, week grid at 768px+); the queue's planned card stating its day instead of asking again; Phase 23 debts closed (plan mutations audited, duplicate ≠ invalid, per-IP throttle on plan writes and `/accounts/sync`).
Non-goals: **fully-unattended publishing** — `script_draft` stays a human gate and ADR-015's locked auto-approval scope is NOT extended (that is a separate, compliance-gated task); AI video from the plan (stock + TTS only); bulk upload; a general calendar/CRM; per-account publishing at DIFFERENT times (`publish_slots.account_id` still unread); caption editing on the approval screen (its own ticket).
Acceptance: nothing publishes without an approval in either mode; a planned instant survives an approval that names no time while an ORDINARY run's behaviour is provably unchanged; a time that passed beyond grace is closed with a real reason and its queued publish cancelled; every guardrail refuses BEFORE any run row exists; tenant isolation on every occurrence read and write; tests + full visual gate green. Reviewers: security-auditor + ux-reviewer + compliance-reviewer (mandatory — autonomy and approval records are touched).

## Phase 25 — Editing the post's text at the approval step · token: START PHASE 25
Why: the AI wrote the caption and hashtags and the operator could only approve or reject them. For the distribution-only user — upload your own video, let Kuyash schedule and publish — that was the single biggest gap (recorded in the checkpoint and in memory).
Goals: per-platform caption + hashtag editing on the approval card and the run screen; the edit written over the same key the publish path already reads (no migration, no second store) with the AI original preserved; a save-time compliance re-gate reusing the SAME SlopScorer and CompliancePolicy thresholds; a publish-time content-hash check that refuses text which reached the row without passing that gate; per-platform limits shown as warnings; the AI disclosure shown but locked.
