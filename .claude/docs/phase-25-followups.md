...
- ~~**The dashboard's accounts card can still take the page down.**~~ **DONE** —
  guarded in the closing round (`Cockpit::accounts()`), with the same third state
  the plan line got: `null`, never an empty list, because an empty list is what
  "No accounts connected yet" is rendered from. The line held: the reads that
  remain (`kpis`, `activeRuns`, `awaiting`, `nextPublish`, `business`) ARE the
  dashboard and still fail loudly. See ADR-024.
